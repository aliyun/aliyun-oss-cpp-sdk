...
TEST_F(BucketBasicOperationTest, BucketLongTermArchiveTest)
{
    //get a random bucketName
    auto bucketName = TestUtils::GetBucketName(testPrefix);
    ...
    //create a new bucket
    Client->CreateBucket(CreateBucketRequest(bucketName, StorageClass::LongTermArchive));
    EXPECT_EQ(TestUtils::BucketExists(*Client, bucketName), true);
    ...
    EXPECT_EQ(bfOutcome.isSuccess(), true);
    EXPECT_EQ(bfOutcome.result().Acl(), CannedAccessControlList::Private);
    EXPECT_EQ(bfOutcome.result().StorageClass(), StorageClass::LongTermArchive);
    ...
    EXPECT_EQ(lbOutcome.result().Buckets().at(0).StorageClass(), StorageClass::LongTermArchive);
